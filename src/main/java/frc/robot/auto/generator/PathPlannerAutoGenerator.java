...
public class PathPlannerAutoGenerator {
  ...
  public PathPlannerAutoGenerator(
    ...
    autoSelector.onChange(
        (cmd) -> {
          if (cmd != null) {
            setStartingPose(cmd);
            ppAuto =
                getPathPlannerAuto(); // because we are composing later, we need to cook a fresh
  ...
  private void setStartingPose(String cmdName) {
    ...
    try {
      ...
      DataLogManager.log(
          "An exception occurred while setting the starting position of a path planner path: "
              + e.getMessage());
      field.setRobotPose(new Pose2d());
    }
  }
  ...
  private void registerCommands() {
    NamedCommands.registerCommand(
        ...
        new ConfigurableShootSequence(
            shooter, undertaker, arm, led, () -> Constants.Auto.CLOSE_A_SHOT_ANGLE));
    NamedCommands.registerCommand(
        "closeBShot",
        new ConfigurableShootSequence(
            shooter, undertaker, arm, led, () -> Constants.Auto.CLOSE_B_SHOT_ANGLE));
    NamedCommands.registerCommand(
        "closeCShot",
        new ConfigurableShootSequence(
            shooter, undertaker, arm, led, () -> Constants.Auto.CLOSE_C_SHOT_ANGLE));
    NamedCommands.registerCommand(
        "closerCShot",
        new ConfigurableShootSequence(
            shooter, undertaker, arm, led, () -> Constants.Auto.CLOSER_C_SHOT_ANGLE));
    NamedCommands.registerCommand(
        "behindCloseBShot",
        new ConfigurableShootSequence(
    ...
    NamedCommands.registerCommand(
        "spinUpFar",
        new AutoSetTargetSpeed(
            shooter, Constants.Auto.SHOT_SPEED, Constants.Auto.SHOT_SPEED)); // non-blocking
    NamedCommands.registerCommand(
        "spinDown", new AutoSetTargetSpeed(shooter, 0, 0)); // non-blocking
  }
    ...
    Command autoCmd =
        new SequentialCommandGroup(
            ...
            new SpinUpAndShoot(
                ...
                () -> Constants.Shooter.SHOOT_SPEED * 0.9),
