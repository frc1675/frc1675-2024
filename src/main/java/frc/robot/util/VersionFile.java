...
public class VersionFile {
  ...
  private VersionFile() {
    File versionFileMini =
        new File(Filesystem.getDeployDirectory(), Constants.Dashboard.VERSION_FILE_NAME);
    ...
    if (versionFileMini.canRead()) {
      try (Scanner reader = new Scanner(new FileReader(versionFileMini))) {
        ...
        DriverStation.reportError(
            "Exception occured while trying to read version string from file", false);
