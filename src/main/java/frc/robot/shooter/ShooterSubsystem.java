...
public class ShooterSubsystem extends SubsystemBase {
  ...
  private PIDController topPidController =
      new PIDController(
          Constants.Shooter.SHOOTER_PID_P,
          Constants.Shooter.SHOOTER_PID_I,
          Constants.Shooter.SHOOTER_PID_D);
  ...
  public boolean isShooterReady() {
    ...
    return Math.abs(targetTopShooterSpeed - speeds[0]) < Constants.Shooter.TARGET_SPEED_ERROR_MARGIN
        && Math.abs(targetBottomShooterSpeed - speeds[1])
            < Constants.Shooter.TARGET_SPEED_ERROR_MARGIN;
  }
